...
export default function RecruitDetailPage() {
    ...
    if (user) {
      ...
      if (p.company_id === user.id) {
        const { data: apps } = await supabase.from('recruit_applications')
          ...
          .eq('posting_id', params.id).order('created_at', { ascending: false })
  ...
  async function submitApplication() {
    ...
    setSending(true)
    const { error } = await supabase.from('recruit_applications').insert({ posting_id: params.id, user_id: user.id, ...form })
    if (error) { alert('エラー: ' + error.message); setSending(false); return }
    await supabase.from('notifications').insert({ user_id: posting.company_id, from_id: user.id, type: 'scout', idea_id: params.id })
    setSending(false)
    setShowForm(false)
    await init()
  }
            ...
            {myApplication ? (
              ...
              <div style={{ background: '#fff', borderRadius: '14px', border: '1.5px solid #1D9E75', padding: '1.5rem' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <textarea value={form.concept} onChange={e => setForm({ ...form, concept: e.target.value })} placeholder="このアイデアが解決する課題と提供する価値" rows={4} style={{ ...inputStyle, resize: 'vertical', lineHeight: '1.7' }} />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={labelStyle}>主な機能・サービス</label>
                  <textarea value={form.features} onChange={e => setForm({ ...form, features: e.target.value })} placeholder="・機能1&#10;・機能2" rows={3} style={{ ...inputStyle, resize: 'vertical', lineHeight: '1.7' }} />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={labelStyle}>収益モデル</label>
                  <textarea value={form.revenue} onChange={e => setForm({ ...form, revenue: e.target.value })} placeholder="例：月額サブスク・手数料など" rows={2} style={{ ...inputStyle, resize: 'vertical', lineHeight: '1.7' }} />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={labelStyle}>差別化の本質</label>
                  <textarea value={form.edge} onChange={e => setForm({ ...form, edge: e.target.value })} placeholder="競合と何が根本的に違うのか" rows={2} style={{ ...inputStyle, resize: 'vertical', lineHeight: '1.7' }} />
                </div>
                <div style={{ marginBottom: '1.25rem' }}>
                  <label style={labelStyle}>企業へのメッセージ</label>
                  <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} placeholder="なぜこの企業と組みたいか、自己PRなど" rows={3} style={{ ...inputStyle, resize: 'vertical', lineHeight: '1.7' }} />
                </div>
              ...
              <button onClick={() => setShowForm(true)} style={{ width: '100%', padding: '14px', borderRadius: '14px', fontSize: '15px', fontWeight: '700', background: '#1D9E75', color: '#fff', border: 'none', cursor: 'pointer' }}>
                📝 この募集にアイデアを応募する
              </button>
            )}
          </div>
