  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f2f5', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '2rem 1.25rem' }}>
        <h1 style={{ fontSize: '22px', fontWeight: '700', marginBottom: '1.5rem', letterSpacing: '-0.5px' }}>法人プロフィール設定</h1>
        ...
        <div style={{ background: '#fff', borderRadius: '14px', border: '0.5px solid rgba(0,0,0,0.1)', marginBottom: '12px', overflow: 'hidden' }}>
          <div onClick={() => bannerInputRef.current?.click()} style={{
            height: '100px', cursor: 'pointer', position: 'relative',
            background: profile?.banner_url ? `url(${profile.banner_url}) center/cover` : '#1a3a5c',
          ...
          <div style={{ padding: '0 1.5rem', display: 'flex', alignItems: 'flex-end', gap: '12px', marginTop: '-28px', marginBottom: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '12px', color: '#6b6b67' }}>{profile?.company_type} · {profile?.industry}</div>
            </div>
          </div>
        </div>
